...
class QLearning:
  def __init__(self, game):
    ...
    self.network = DeepQNetwork(state_size=self.game.state_size, action_size=self.game.action_size)
  ...
  def train(self, dt):
    ...
    if np.random.rand() < 0.05:
      ...
      cur_action = np.argmax(self.network(np.atleast_2d(cur_state)))
    ...
    if len(self.experiences) >= self.batch_size * 4:
      ...
      targets = rewards + self.gamma * np.max(self.network(np.atleast_2d(next_states)), axis=1)
      targets = tf.convert_to_tensor(targets, dtype="float32")

      # calculate loss
      with tf.GradientTape() as tape:
        predictions = tf.math.reduce_sum(
          self.network(np.atleast_2d(cur_states).astype("float32")) * tf.one_hot(cur_actions, self.game.action_size),
          axis=1)
  ...
  def test(self):
    next_action = np.argmax(self.network(np.atleast_2d(self.game.get_state())))
